...
def ask_gemini(prompt: str) -> str:
  ...
  try:
    ...
    if e.code == 429:
      return (
          "Gemini is currently rate-limited or over quota.\n"
          "Please try again later or increase your Gemini quota."
      )
    else:
...
def summarize_last_notes(user_id: int, limit: int = 10) -> str:
  ...
  # Use whichever AI provider is configured (OpenAI or Gemini)
...
def help_text() -> str:
  return (
      ...
      "/ask <question> - AI answer (OpenAI if set, else Gemini)\n"
      "/ask_openai <q> - force OpenAI (if configured)\n"
      "/ask_gemini <q> - force Gemini (if configured)\n"
...
def handle_text_message(chat_id: int, message_id: int, text: str, first_name: str | None):
  ...
  if text == "/ask":
    send_message(
        chat_id,
        "Usage: /ask <question>\nExample: /ask How does DynamoDB work?",
    )
    return
